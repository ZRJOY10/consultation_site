  ...
  return (
    <>
      ...
      <div className={`hidden lg:block border-b backdrop-blur-sm transition-colors duration-300 ${
        isDark ? 'bg-[#080e1a]/80 border-white/[0.04]' : 'bg-copper-50/80 border-copper-200/30'
      }`}>
        <div className={`max-w-7xl mx-auto px-6 py-2 flex justify-between items-center text-xs ${
          ...
          <div className="flex items-center gap-4">
            ...
            <span className={isDark ? 'text-sky-400/70' : 'text-copper-600'}>Free consultation available</span>
          </div>
        </div>
      </div>
      ...
      <header className={`sticky top-0 z-50 transition-all duration-500 ${
        scrolled
          ? isDark
            ? 'bg-[#0c1222]/95 backdrop-blur-xl shadow-lg border-b border-white/[0.04]'
            : 'bg-white/90 backdrop-blur-xl shadow-md border-b border-copper-200/20'
          : 'bg-transparent'
      }`}>
            ...
            <Link to="/" className="flex items-center gap-3 group">
              <div className="relative w-10 h-10">
                ...
                <div className={`absolute inset-0 rounded-xl flex items-center justify-center ${
                  isDark ? 'bg-gradient-to-br from-sky-600 to-sky-500' : 'bg-gradient-to-br from-copper-600 to-copper-500'
                }`}>
                        ...
                        {activeDropdown === link.label && (
                          <motion.div
                            ...
                            className={`absolute top-full left-0 mt-2 w-56 backdrop-blur-xl rounded-xl shadow-lg overflow-hidden ${
                              isDark ? 'bg-[#111827]/95 border border-white/[0.07]' : 'bg-white/95 border border-copper-200/30'
                            }`}
                          >
                            {link.sub.map((sub) => (
                              <Link
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`p-2.5 rounded-xl transition-all duration-300 ${
                  isDark
                    ? 'bg-white/5 border border-white/10 text-slate-400 hover:text-amber-400 hover:bg-white/10'
                    : 'bg-copper-50 border border-copper-200/30 text-copper-600 hover:text-indigo-600 hover:bg-copper-100'
                }`}
          ...
          {mobileOpen && (
            <motion.div
              ...
              className={`xl:hidden backdrop-blur-xl overflow-hidden ${
                isDark ? 'bg-[#0c1222]/98 border-t border-white/[0.04]' : 'bg-white/98 border-t border-copper-200/20'
              }`}
            >
                  ...
                  >
                    <Link
                      ...
                      className={`block px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                        isActive(link.path)
                          ? isDark ? 'text-white bg-sky-600/10 border border-sky-500/20' : 'text-copper-900 bg-copper-600/10 border border-copper-500/20'
